...
ggplot(cbind(fit.inla.gaussian$summary.fitted.values, pit.g),
       aes(x=mean, y=pit.g)) +
  geom_point() + labs(title="PIT values over posterior mean fitted values",
...
resids = qg.data.gg.inds$surv.ind.to.ad -
  fit.inla.gaussian$summary.fitted.values$`mean`
par(mfrow=c(1,1))
plot(resids, main="Residuals of Gaussian model", ylab="")
df.resid = qg.data.gg.inds$surv.ind.to.ad-fit.inla.gaussian$summary.fitted.values
rownames(df.resid) = 1:nrow(df.resid)
ggplot(data=df.resid, aes(x=as.numeric(row.names(df.resid)), y=mean)) +
  geom_errorbar(aes(ymin=`0.025quant`, ymax=`0.975quant`, color="95% credible interval")) +
  geom_point(aes(color="Mean residuals")) + labs(title="Residuals of Gaussian model", x="Index", y="Residuals") +
  scale_color_manual(name="", breaks=c("95% credible interval", "Mean residuals"), 
                     values=c("95% credible interval"='darkgrey', "Mean residuals"='darkred'))
