...
plot(fit.inla.logit$summary.linear.predictor[, 1],
     ...
     main="Logit (x) versus Probit (y)", xlab="",ylab=""
)
abline(0,1)
plot(fit.inla.gaussian$summary.linear.predictor[, 1],
     fit.inla.probit$summary.linear.predictor[, 1],
     main="Gaussian (x) versus Probit (y)", xlab="", ylab=""
)
abline(0,1)
plot(fit.inla.gaussian$summary.linear.predictor[, 1],
...
## Just plot PIT also shows a clear separation in the "residuals"
plot(pit.g)
...
resids = qg.data.gg.inds$surv.ind.to.ad - fit.inla.gaussian$summary.fitted.values$`0.5quant`
plot(resids)

est.mean = mean(qg.data.gg.inds$surv.ind.to.ad)
est.sd = sd(qg.data.gg.inds$surv.ind.to.ad)
# sum(fit.inla.gaussian$summary.hyperpar$`0.5quant`^2)
qqplot(qnorm(ppoints(n.obs), est.mean, est.sd), resids)
...
lol_test <- function(fit, param){
  paste(mean(get(fit, param)), get_mode(get(fit, param)), sd(get(fit,param)), sep=", ")
} # ## # # TODO figure a better way to do this dynamically...

lol_test(fit.inla.probit, h2.latent)

knitr::kable(data.frame(
  mean(fit.inla.logit$h2.latent),
  mean(fit.inla.probit$h2.latent),
  mean(fit.inla.gaussian$h2.latent)
  ),
  format="html",
  col.names=c("Logit", "Probit", "Gaussian")
  )
```
