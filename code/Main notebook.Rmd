...
multiple.h2.dev <- function(sigma.scale, ntimes, ...){
  ...
  for(i in 1:ntimes){
    cat(paste0("\n [Run ", i ,"/", ntimes, "]\n"))
    # res <- list(data=data.frame(latent=rnorm(n.sigma),
    #                             estimate=rnorm(n.sigma),
    #                             true.vals=rnorm(n.sigma)))
  ...
  ggplot(plot.data, aes(x=xax)) +
    geom_pointrange(aes(ymax=top, ymin=btm, y=mid, color=model)) + scale_x_log10()
}

multiple.h2.dev("log",3)
