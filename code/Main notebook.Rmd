...
First, rerun with $\eta_i=a_i$, i.e. without residuals on underlying scale. Similar results.
...
(For now, we only consider application data, but it's easy to do the same for the simulation case)
...
Then, compute the 3 different approaches with `QGglmm` (this is quite slow).

```{r}
h2.psi <- data.frame(application.bayesian=
                       get.h2.from.qgparams(fit.inla.probit, "binom1.probit",
                                        n.samples, averaging = T,
                                        averaging.mode.only = F)
)
h2.psi$application.frequentist <- get.h2.from.qgparams(fit.inla.probit,
                                                       "binom1.probit",
                                                       n.samples,
                                                       averaging = T,
                                                       averaging.mode.only = T)
h2.psi$application.noavg <- get.h2.from.qgparams(fit.inla.probit,
                                                       "binom1.probit",
                                                       n.samples,
                                                       averaging = F)
# Link variance scaling for reference
h2.psi$application.phi <- heritability$probit.scaled
...
ggplot(data=melt(h2.psi)) +
  geom_density(aes(x=value, fill=variable), alpha=0.5) +
  scale_x_log10() +
  scale_fill_manual(
    values = c("#FFA600", "#FA2A00", "#7C43A6", "#0096FF"),
    labels = c(
...
How does the Threshold-transformed Gaussian heritability perform compared to the posteriors for the probit model?
...
ggplot(
  melt(heritability[, c("gaussian.liability", "probit.latent")]),
