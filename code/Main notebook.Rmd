...
simulated.heritability <- function(NeNc = 0.5, idgen = 100, nGen = 9, sigmaA = 0.8,
                                   ...
                                   dichotomize = "round", pc.prior = NA, probit.model = F, DIC=F) {
  ...
  if (dichotomize == "binom1.logit") {
    ...
    simulated.response <- ifelse(linear.predictor(u, simulated.d.ped) <= cutoff, 0, 1)
  ...
  if(DIC){
    simulated.fit.inla <- inla(
      ...
      data = simulated.d.ped, control.compute=list(dic=T)
      )
  }
  else{
    simulated.fit.inla <- inla(
  ...
  if (probit.model) {
    fit.probit <- inla(
      ...
      data = simulated.d.ped, control.compute = list(return.marginals.predictor = T, dic=T)
    )
  } else {
    fit.probit <- NULL
  }
...
plot.h2.deviation <- function(dichotomize = "round",
                              ...
                              dynamic.priors = F, simulated.formula = NULL, Ve=NULL, fixedeffects = FALSE) {
  if (sigma.scale == "log") {
  ...
  for (sigmaA in sigmaA.list) {
    ...
    if(fixedeffects){
      Ve <- Ve + Ve/(result$p * (1-result$p))
      posterior <- get.h2(result$fit, 10000, include.fixed=T)
    } else{
      posterior <- result$heritability
    }
    
    simulation.h2.true <- sigmaA/ (sigmaA + Ve)
    # cat("[Debug]: sigmaA", sigmaA, "Ve", Ve, "h^2 true", simulation.h2.true, "\n")
...
plot.qgglmm.heritability(h2.psi.sim2, "simulation",
                         SAVE.PLOT, plot.title=TeX("Simulation, $\\sigma^2_A=0.5$"),
                         fn.append="va0.1")
...
First, we fit the simulation probit model and get simulation-based heritability on all scales.
...
print_heritability_table <- function(digits, h2appl, h2sim) {
  ...
  main <- paste(
    "Gaussian $h^2_{obs}$ &", print_one_metric(h2appl, "gaussian", digits),
    "&", print_one_metric(h2sim, "gaussian", digits),
    "\\\\ \n",
    "Gaussian $h^2_{liab}$ &", print_one_metric(h2appl, "gaussian.liability", digits),
    "&", print_one_metric(h2sim, "gaussian.liability", digits),
    "\\\\ \n",
    "Probit $h^2_{lat}$ &", print_one_metric(h2appl, "probit.latent", digits),
    "&", print_one_metric(h2sim, "probit.latent", digits),
    "\\\\ \n",
    "Probit $h^2_{\\Phi}$ &", print_one_metric(h2appl, "probit.scaled", digits),
    "&", print_one_metric(h2sim, "probit.scaled", digits),
    "\\\\ \n",
    "Probit $h^2_{\\Psi}$ &", print_one_metric(h2appl, "probit.qgglmm", digits),
    "&", print_one_metric(h2sim, "probit.qgglmm", digits),
    "\n"
  )
...
print_heritability_table(4, heritability, heritability.sim)
```
...
data.frame(
  ...
  Probit = simulation.res2$fit.probit$dic$dic, row.names = "Deviance Information Criteria"
)
```

# Fixed effects for simulation

Okay, let's try to add sex covariate to linear predictor. Assume that (but double-check with someone!)
$$
\operatorname{Var}[\beta_{\text{sex}}] = \frac{\operatorname{Var}[\varepsilon]}{\operatorname{Var}[\mathbf{x}_{\text{sex}}]}= \frac{\sigma^2_E}{p(1-p)}.
$$
...
m <- plot.h2.deviation(
  ...
  SAVE.PLOT = T, plot.fn = "fixedeffects_beta100", sigma.scale = "log",
  lin.pred = linear_predictor_fixedeffects, simulated.formula = simulated.formula.fixedeffects,
  Ve=1, fixedeffects = TRUE
)
m$p
...
m2 <- plot.h2.deviation(
  ...
  SAVE.PLOT = T, plot.fn = "fixedeffects_beta10", sigma.scale = "log",
  lin.pred = linear_predictor_fixedeffects, simulated.formula = simulated.formula.fixedeffects,
  Ve=1, fixedeffects = TRUE
)
m2$p
...
m3 <- plot.h2.deviation(
  dichotomize = "round", title = TeX("$\\beta_{sex}=10$"),
  SAVE.PLOT = T, plot.fn = "fixedeffects_beta10_unbalanced", sigma.scale = "log",
  lin.pred = linear_predictor_fixedeffects, simulated.formula = simulated.formula.fixedeffects,
  Ve=1, fixedeffects = TRUE
)
...
sum(linear_predictor_fixedeffects(u,simulated.d.ped) <=0)/900


# The one with fixed effects is very "janky", let's average over several runs.
average.h2.deviation <- function(){
...
# Variations to models

This section implemented the following:

- Include IID N(0,1) noise in Probit simulation models
- Overdispersion in simulation models, i.e. $V_E >1$.
...
```{r}
# Add IID N(0,1)
alternative.probit.sim <- function(sigmaA, linear.predictor, sim.formula=NULL){
  #' Init
  idgen <- 100
  NeNc <- 0.5
  
  #' Generate pedigreee
  ped = generatePedigree(nId=idgen, nGeneration=9, nFather=idgen*NeNc,
                         nMother=idgen*NeNc)
  ped <- ped[, c(1,3,2,5)]
  ...
  simulated.d.ped$ind <- 1:nrow(simulated.d.ped)
  ...
  # ' INLA fitting
  if(is.null(simulated.formula)){
    simulated.formula <- simulated.response ~ f(id, model = "generic0", Cmatrix = simulated.Cmatrix,
        constr = F, # Shouldn't matter
        hyper = list(prec = list(initial = log(1 / 10), prior = "pc.prec",
                                 param = c(1, 0.05)))) +
      f(ind, model="iid", constr = T#,
        # hyper=list(prec=list(initial = 0, fixed = T)) # Comment to use default
        )
  }
  else{
    environment(simulated.formula) <- environment()
...
```{r}
sim.fit.probit <- alternative.probit.sim(0.5, function(u,.) u+rnorm(length(u), 0, sqrt(9)))

testdf = get.h2.from.qgparams(sim.fit.probit,"binom1.probit",10000)

ggplot(data=data.frame(x=testdf)) + geom_density(aes(x=x))

## Plot posterior QGglmm density? And show 'true' value threshold-scaled

```` 

## Overdispersion to simulation models

```{r}
# I think we have all functions we need to run this?
# Remember to pass Ve in the function though
```