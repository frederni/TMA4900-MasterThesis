...
plot.markov.result("heritabilities_SSH_binom1.probit.Rdata", 10000,
                   plot.title="Heritability for Probit model", saveplot=T)
plot.markov.result("heritabilities_SSH_binom1.logit.Rdata", 10000,
                   plot.title="Heritability for Logit model", saveplot=T)
...
if(SAVE.PLOT) ggsave("../figures/simulation_deviance_from_true_h2.pdf")
...
threshold.scaled.h2 <- 1/threshold.scaling.param(sim.result$p) * sim.result$heritability
simulation.h2.true <- v_a/(v_a+0) # Dependent on linear predictor, e.g. sigmaA/(sigmaA+1)
...
ped0 <- generatePedigree(nId = 100, nGeneration = 9, 
                         nFather = 100 * 0.5, nMother = 100 * 0.5)
pedigree <- ped0[ , c(1,3,2)]
names(pedigree) <- c("id", "dam", "sire")
u <- rbv(pedigree, 0.1)
simulated.d.ped <- nadiv::prepPed(pedigree)
A.matrix = nadiv::makeA(simulated.d.ped)
A.diag = diag(A.matrix)
A.nondiag = A.matrix
diag(A.nondiag) = NA
{
  par(mfrow=c(2,1))
  truehist(A.nondiag@x, main="Off-diagonal elements of A")
  plot(A.diag, main="Diagonal elements of A")  
}

```
This is an interesting observation. First, most of the off-diagonal elements are very close to 0, indicating almost no realtedness. Secondly, not all diagonal elements are themselves 1. What does this mean?
