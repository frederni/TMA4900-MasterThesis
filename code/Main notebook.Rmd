...
Let's grid-plot densities. This might have to be moved later but

```{r}
# TODO: If time, consider working around facet_wrap instead, as it's more readable
p1 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian.latent", "probit.latent")]), aes(x=value,fill=variable), alpha=0.5) +
  theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{G,obs}$ vs. $h^2_{B,L}$"))
p2 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian.liability", "probit.latent")]), aes(x=value,fill=variable), alpha=0.5) +
  theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{G,liab}$ vs. $h^2_{B,L}$"))
p3 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian.latent", "probit.qgglmm")]), aes(x=value,fill=variable), alpha=0.5) +
  theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{G,obs}$ vs. $h^2_{B,\\Psi}$"))
p4 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian.liability", "probit.qgglmm")]), aes(x=value,fill=variable), alpha=0.5) +
  theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{G,liab}$ vs. $h^2_{B,\\Psi}$"))
p5 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian.latent", "probit.scaled")]), aes(x=value,fill=variable), alpha=0.5) +
  theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{G,obs}$ vs. $h^2_{B,\\Phi}$"))
p6 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian.liability", "probit.scaled")]), aes(x=value,fill=variable), alpha=0.5) +
  theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{G,liab}$ vs. $h^2_{B,\\Phi}$"))

plot_grid(p1,p3,p5,p2,p4,p6,ggplot()+theme_void(),
          get_legend(
...
if(SAVE.PLOT) ggsave("../figures/application_gaussian_vs_binom.pdf")
...
plot.markov.result("heritabilities_SSH_binom1.probit.Rdata", 10000,
  plot.title = "Observation scale transformations for probit model", saveplot = T
)
