...
ggplot(df.latent.h2, aes(x = samples, fill = Model)) +
  ...
  ylab("Density")
if(SAVE.PLOT) ggsave("../figures/probit-logit-similarity.pdf")
```
Generally we don't see much difference. Now we want to see how the gaussian one preforms. Preliminary results..
...
df.transformed.h2 <- data.frame(
  ...
  Model = c(
    ...
    rep("Gaussian (no transformation)", length(heritability$gaussian))
  )
)
...
compute.probit.transforms <-function(df.latent.h2){
...
plot.markov.result("../data/heritabilities_SSH_binom1.probit.Rdata", 10000,
  plot.title = "Transformations for probit model", saveplot = T
)

plot.markov.result("../data/simulation_heritabilities_SSH_binom1.probit.Rdata", 10000,
  plot.title = "Simulation: Transformations for probit model", saveplot = T
)

# Currently not using logit model
# plot.markov.result("../data/heritabilities_SSH_binom1.logit.Rdata", 10000,
...
oldPlottingHeritability <- function() {
  ...
  par(mfrow = c(2, 2))
  load("../data/heritabilities_SSH_binom1.logit.Rdata")
  ...
  load("../data/heritabilities_SSH_binom1.probit.Rdata")
  truehist(heritability_averaged$h2.obs,
    main = "Bayesian",
    xlab = expression(h[obs]^2)
  )
...
Seems to fit generally well here. Let's quantitatively look into estimates for different sigmaA:
...
```{r}
# Try to re-run with another linear predictor, similar result
...
The observation-scale heritability actually looks very good.


```{r}
ggplot(
  melt(heritability[, c("gaussian", "logit.qgglmm", "probit.qgglmm")]),
  aes(x = value, fill = variable)
) +
  geom_density(alpha = 0.5) +
...
ggplot(
  melt(heritability[, c("gaussian", "logit.scaled", "probit.scaled")]),
  aes(x = value, fill = variable)
) +
  geom_density(alpha = 0.5) +
...
First, we look at application study. Let's grid-plot densities.

```{r}
# TODO: If time, consider working around facet_wrap instead, as it's more readable
plot_grid_of_heritability <- function(heritability, SAVE.PLOT, plot.fn, colorscheme=NA){
  p1 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian", "probit.latent")]), aes(x=value,fill=variable), alpha=0.5) +
    theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{obs}$ vs. $h^2_{lat}$")) +
    if(!is.na(colorscheme)) scale_fill_brewer(palette=colorscheme)
  p2 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian.liability", "probit.latent")]), aes(x=value,fill=variable), alpha=0.5) +
    theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{liab}$ vs. $h^2_{lat}$"))+
    if(!is.na(colorscheme)) scale_fill_brewer(palette=colorscheme)
  p3 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian", "probit.qgglmm")]), aes(x=value,fill=variable), alpha=0.5) +
    theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{obs}$ vs. $h^2_{\\Psi}$"))+
    if(!is.na(colorscheme)) scale_fill_brewer(palette=colorscheme)
  p4 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian.liability", "probit.qgglmm")]), aes(x=value,fill=variable), alpha=0.5) +
    theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{liab}$ vs. $h^2_{\\Psi}$"))+
    if(!is.na(colorscheme)) scale_fill_brewer(palette=colorscheme)
  p5 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian", "probit.scaled")]), aes(x=value,fill=variable), alpha=0.5) +
    theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{obs}$ vs. $h^2_{\\Phi}$"))+
    if(!is.na(colorscheme)) scale_fill_brewer(palette=colorscheme)
  p6 <- ggplot() + geom_density(data=melt(heritability[, c("gaussian.liability", "probit.scaled")]), aes(x=value,fill=variable), alpha=0.5) +
    theme(legend.position="none",axis.title=element_blank()) +labs(title=TeX("$h^2_{liab}$ vs. $h^2_{\\Phi}$"))+
    if(!is.na(colorscheme)) scale_fill_brewer(palette=colorscheme)
  p <- plot_grid(p1,p3,p5,p2,p4,p6,ggplot()+theme_void(),
            get_legend(
...
plot_grid_of_heritability(heritability, TRUE, "application_gaussian_vs_binom.pdf")
# For simulation:
plot_grid_of_heritability(heritability.sim, TRUE, "simulation_gaussian_vs_binom.pdf",'Accent')
