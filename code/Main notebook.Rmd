...
marginal.latent.alternative <- function(fit, nsamples){
  ...
  for(predictor in names(fit$marginals.linear.predictor)){
    ...
    out_transpose = cbind(out_transpose, predictor_samples) # testing to see if this is culprit
  }
...
t_before = Sys.time()
transformed_heritability = new.h2.transf(fit.inla.logit, "binom1.logit", 100)
