...
simulation.res <- plot.h2.deviation(plot.fn = "round", SAVE.PLOT=F)
simulation.res$p
...
tmp = simulated.heritability(linear.predictor=function(u) u+rnorm(length(u)),
                             simulated.formula = simulated.formula)

paste(c("Resid", "ID"), 1/tmp$fit$summary.hyperpar$mean)
summary(tmp$fit)


tmp$fit$summary.fixedped0$sex = ped0$sex -1 # Binarize :)
linear_predictor_fixedeffects <- function(u){
