...
simulated.heritability <- function(NeNc=0.5, idgen=100, nGen = 9, sigmaA=0.4,
                                   simulated.response=NA, simulated.formula=NA){
...
## First model: Only use breeding values and some residuals
simulated.mod0 <- u + rnorm(length(u), sd=1) # Question: Can I choose SD for the residuals? What can I base it on if not?
sigmoid.scale <- function(x) exp(x)/(exp(x)+1)
simulated.mod0.sc <- sigmoid.scale(simulated.mod0) # Pi expression
simulated.response <- rbinom(length(u), size=1, prob=simulated.mod0.sc)
...
rownames(simulated.fit.inla$summary.hyperpar)
simulated.fit.inla$mode$mode.status
plot(simulated.fit.inla$summary.fitted.values$mean)

test = get.h2(simulated.fit.inla, 10000)
mean(test)
list(mean=mean(test), standard.deviation=sd(test), quantiles=quantile(test, probs=c(0.05, 0.5, 0.95)))
