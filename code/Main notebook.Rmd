...
get.h2.from.qgparams <- function(inla.fit, modelname, n, n.obs=nrow(d.ped)){
  ...
  marginal.mode = inla.posterior.marginal.latent.mode(inla.fit)
  vp.samples = 0
  ...
  # browser()
  kwargs = list(verbose=T, model = modelname, predict = marginal.mode)
  out = mapply(QGparams, NULL, va.samples, vp.samples, MoreArgs = kwargs)
  return(out)
  h2.getter = function(...){
    cat(...)
    get("h2.obs", suppressWarnings(QGparams(...)))
    }
  out = mapply(h2.getter,  MoreArgs=kwargs) # va.samples, vp.samples, modelname,
  return(out)
}
...
t_before = Sys.time()
new.h2.transf(fit.inla.logit, "binom1.logit", 100)
