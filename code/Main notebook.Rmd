...
# R code acknowledgements {.unlisted .unnumbered}
...
- build a pedigree structure from the denormalized table with \texttt{prepPed}.
...
## Deriving *A* {.unlisted .unnumbered}
...
```
# Latent heritability {.unlisted .unnumbered}
Below is a function to get $h^2_\text{obs}$, $h^2_\text{lat}$ and $h^2_\Phi$ based on a fitted model. 
...
# Simulation data {.unlisted .unnumbered}
...
Seems to fit generally well here. We quantitatively look into estimates for different $\sigma^2_A$:

## Performance over varying $V_A$ {.unlisted .unnumbered}
...
multiple.h2.dev <- function(sigma.scale, ntimes,
                            title = "Simulation heritability", ...){
  #' Run plot_h2_deviation `ntimes` and report mean
  #' 
  #' Repeats the runs several times, and outputs error plot,
  #' (pointrange) with mean +- SD
  #' @param sigma.scale either 'log' or 'small', like in other fnc.
  #' @param ntimes Number of repeated runs
  #' @param title Legend title, character
  #' @param ... Additional parameters to plot_h2_deviation
...
## Without residual in linear predictor {.unlisted .unnumbered}
...
# Residual analyses on Gaussian model {.unlisted .unnumbered}
...
# Transformations of heritability {.unlisted .unnumbered}
...
## Compare different scales {.unlisted .unnumbered}
...
# Fixed effects for simulation {.unlisted .unnumbered}
...
## Fixed effect model performance {.unlisted .unnumbered}
...
plot.fixedeffects.h2 <- function(sA, .dichotomize, include.fixed = T,
  ...
  p <- ggplot(melt(h2.sim.fixed[, c("gaussian", "probit.qgglmm")])) +
    ...
    xlim(c(0,1)) + ylim(c(0,10)) # TODO might have to finetune
  if (SAVE.PLOT) {
    p.legend <- cowplot::get_legend(p)
...
# IID noise to probit simulation {.unlisted .unnumbered}
...
overdisperion_wrapper <- function(nsamps, vA, vE, SAVE.PLOT) {
  ...
  true.h2.overdisp <- 1 / threshold.scaling.param(overdisp.p) * 
    vA / (vA + vE)
  ...
  curr.plot <- ggplot(data = melt(df.probit.comp)) +
    ...
    scale_color_manual(
      name = "", labels = c(TeX("True $h^2_{obs}$")),
...
# Illustrative figures {.unlisted .unnumbered}
...
ggplot(eta_df) +
  ...
  geom_line(
    data = eta_df[eta_df$elem == "z", ], aes(x = value, color = elem),
