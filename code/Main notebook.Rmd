...
simulated.heritability <- function(NeNc = 0.5, idgen = 100, nGen = 9,
  ...
  if (dichotomize == "binom1.logit") {
    ...
    cutoff <- quantile(linear.predictor(u, simulated.d.ped), 1-dichotomize)
    simulated.response <- ifelse(
      linear.predictor(u, simulated.d.ped) <= cutoff, 0, 1
    )
  } else {
...
plot.h2.deviation <- function(
  ...
  p <- ggplot(data = res, aes(x = sigmaA)) +
    ...
    geom_line(aes(y = true.vals, color = "true"),size=1.5) +
    geom_line(aes(y = estimates, color = "liab"),size=1.5) +
    ...
    scale_color_manual(
      name = title,
      values = c("true" = "darkred", "liab" = "steelblue",
                 "obs" = "chartreuse3"),
