...
plot.h2.deviation <- function(dichotomize = "round",
  ...
  p <- ggplot(data = res, aes(x = sigmaA)) +
    ...
    geom_line(aes(y = latent, color = "Fitted h^2 (Latent)")) +
    xlab(TeX("$\\sigma^2$")) +
    ylab(TeX("$h^2$")) +
    scale_x_log10() +
    scale_color_manual(
...
simulation.res <- plot.h2.deviation(plot.fn = "round", SAVE.PLOT=T)
simulation.res$p

### Plot for small values og sigmaA, but finer grid
simulation2.res <- plot.h2.deviation(SAVE.PLOT = F, sigma.scale = "small")
simulation2.res$p

### Standard plotting with different dichotomization techniques
plot.h2.deviation(
  dichotomize = "binom1.logit",
  title = "Simulation heritability,\ndichotomized as binomial",
  plot.fn = "binom"
)
...
df.transformed.h2.threshold = rbind(
  df.transformed.h2, data.frame(samples = heritability$gaussian.latent *
                                  1/threshold.scaling.param(
                                    mean(qg.data.gg.inds$surv.ind.to.ad)),
                                Model = rep("Gaussian (Threshold scaled)",
                                            length(heritability$gaussian.latent))
                                )
)


ggplot(df.transformed.h2.threshold, aes(x = samples, fill = Model)) +
  geom_density(alpha = 0.5) +
  ggtitle("Threshold model with respect to h_obs: t^2/(p(1-p))") +
  xlab("Heritability") +
  ylab("Density") +
  xlim(c(0, 0.1)) #+ scale_x_log10()
...
Now, let's instead try to include the fixed effect variance into the denominator of the heritability. Maybe that improves heritability?

```{r}

# Let's run that in another notebook to avoid confusion
par(mfrow=c(1,2))
truehist(get.h2(fit.inla.gaussian,10000,include.fixed = T),main="Fixed effects")
truehist(get.h2(fit.inla.gaussian,10000),main="Not")
