...
We also implement a method to average over several runs, reducing stochasticity.
...
plot.fixedeffects.h2 <- function(sA, .dichotomize, include.fixed = T,
  ...
  h2.sim.fixed <- get.all.heritabilities(fits$fit, fits$fit.probit,
                                         fits$p, n.samples,
                                         fixed = FALSE)
  h2.true.fixed <- sA / (sA + sE + beta^2 * 
                           var(fits$simulated.response)
                         ) *
    1 / threshold.scaling.param(fits$p)

  p <- ggplot(melt(h2.sim.fixed[, c("gaussian", "probit.qgglmm")])) +
    geom_density(aes(x = value, fill = variable), alpha = 0.5) +
    geom_vline(aes(xintercept = h2.true.fixed, color = "true"),
      lwd = 1,
      linetype = "dashed"
    ) +
    scale_color_manual(
      name = "", labels = TeX("True $h^2_{obs}$"),
